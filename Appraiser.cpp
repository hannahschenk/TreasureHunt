...
double Appraiser::appraiseCoin(Coin** aCoin) {
	...
	if (chance <= 2) {
		//centennial
		Coin* temp = new CentennialRoyal(**aCoin);
		delete *aCoin;
		*aCoin = temp;
		(*aCoin)->appraise((rand() % 5 + 1) * 100000);  //100,000 to 500,000 dollars

		return (*aCoin)->getValue();
	}
	else if (chance >= 3 && chance <= 33) {
		//escudos
		Coin* temp = new Escudos(**aCoin);
		delete *aCoin;
		*aCoin = temp;
		(*aCoin)->appraise((rand() % 5 + 5) * 10000);  //50,000 to 100,000 dollars
